...
@WebMvcTest(controllers = OsgForgotPasswordController.class)
...
class OsgForgotPasswordControllerAnonymousIntegrationTest
{
    @Autowired
    ...
    @Test
    void sendCodeShouldAllowAnonymousRequestOnMentorRoute() throws Exception
    {
        doNothing().when(passwordService).sendResetCode("test@example.com");

        mockMvc.perform(post("/mentor/forgot-password/send-code")
