...
public class OsgMentorAuthController
{
    @Autowired
    ...
    public AjaxResult login(@RequestBody LoginBody loginBody)
    {
        ...
        SysUser user = mentorAccessService.findUserByLogin(loginName);
        if (user != null && !mentorAccessService.hasMentorAccess(user))
        {
            return AjaxResult.error("该账号无导师端访问权限");
        }
    ...
    public AjaxResult getInfo()
    {
        ...
        AjaxResult ajax = AjaxResult.success();
        ajax.put("user", user);
        ajax.put("roles", mentorAccessService.buildPortalRoles(user));
