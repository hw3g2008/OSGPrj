...
> **来源**：`verification-before-completion` skill 核心内容提取
>
> **用途**：`fix` skill Phase 4 的执行参考。被 `fix` SKILL.md Phase 4 调用。
>
> **职责**：在声称"完成/修好了/OK"之前必须调用的验证技能。任何代码/配置修改后的总结性回复前，必须通过此验证。
