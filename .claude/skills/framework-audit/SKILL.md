...
- 任何 `.claude/skills/`、`.claude/agents/`、`.claude/project/`、`.claude/commands/` 下的文件被修改后
...
audit_files:
  ...
  skills:
    - ".claude/skills/*/SKILL.md"       # 所有 Skill 文件
  commands:
    - ".claude/commands/*.md"           # 所有命令文件
  agents:
    - ".claude/agents/*.md"             # 所有 Agent
```
...
1. `templates/ticket.yaml` — type 注释枚举
2. `skills/ticket-splitter/SKILL.md` — 概览或流程中的枚举
3. `skills/deliver-ticket/SKILL.md` — 流程分支覆盖
4. `skills/verification/SKILL.md` — `can_claim_done()` 分支覆盖
5. `CLAUDE.md` — 角色表
