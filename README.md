...
## Docker Runtime（OSG 扩展）

本仓库在原始 RuoYi 基础上，规划了 OSG 的 Docker 同构运行方案（test/prod）。

- 设计文档：`docs/plans/2026-03-01-docker-runtime-architecture-design.md`
- 状态：Phase 1 已落地（需在目标环境执行一次全量验收）
- 执行入口：
  - `bash bin/prepare-mysql-init.sh`
  - `bash bin/docker-env-up.sh test --profile deps`
  - `bash bin/docker-env-up.sh test --profile core`
- 收尾入口：`bin/final-closure.sh`（支持 `--backend-policy docker_only`）
