<template>
  <a-layout class="main-layout">
    <a-layout-sider v-model:collapsed="collapsed" collapsible theme="dark">
      <div class="logo"><span v-if="!collapsed">OSG 班主任端</span><span v-else>OSG</span></div>
      <a-menu v-model:selectedKeys="selectedKeys" theme="dark" mode="inline" @click="handleMenuClick">
        <a-menu-item key="dashboard"><template #icon><DashboardOutlined /></template><span>工作台</span></a-menu-item>
        <a-menu-item key="classes"><template #icon><AppstoreOutlined /></template><span>班级管理</span></a-menu-item>
        <a-menu-item key="students"><template #icon><TeamOutlined /></template><span>学员管理</span></a-menu-item>
        <a-menu-item key="mentors"><template #icon><UserSwitchOutlined /></template><span>导师管理</span></a-menu-item>
        <a-menu-item key="schedule"><template #icon><CalendarOutlined /></template><span>排课总览</span></a-menu-item>
        <a-menu-item key="reports"><template #icon><BarChartOutlined /></template><span>学情报告</span></a-menu-item>
        ...
        <div class="header-right">
          <a-dropdown>
            <a class="user-dropdown" @click.prevent>
              <a-avatar :size="32">{{ userInfo?.nickName?.[0] || 'L' }}</a-avatar>
              <span class="user-name">{{ userInfo?.nickName || '班主任' }}</span>
            </a>
            <template #overlay>
              <a-menu>
...
import { DashboardOutlined, AppstoreOutlined, TeamOutlined, UserSwitchOutlined, CalendarOutlined, BarChartOutlined, UserOutlined } from '@ant-design/icons-vue'
...
.logo { height: 64px; display: flex; align-items: center; justify-content: center; color: #fff; font-size: 16px; font-weight: 600; background: rgba(255, 255, 255, 0.1); }
