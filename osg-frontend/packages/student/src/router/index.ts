import { createRouter, createWebHistory } from 'vue-router'
import type { RouteRecordRaw } from 'vue-router'
import MainLayout from '@/layouts/MainLayout.vue'
import { COMING_SOON_TOAST, isStudentPathAvailable, normalizeStudentPath } from '@/navigation/access'
import { PHASE1_DEFAULT_PATH } from '@/navigation/phase1'
import { getToken, getUser } from '@osg/shared/utils'
import { localizeText } from '@osg/shared'

const STUDENT_COMING_SOON_ROUTE_NAME = 'StudentComingSoon'
const STUDENT_COMING_SOON_SUBTITLE = "当前页面不在本次学生端交付范围内。" // i18n-skip-line: route meta title

function readSingleQueryValue(value: unknown): string | undefined {
  if (typeof value === 'string' && value.trim()) {
    return value
  }

  if (Array.isArray(value)) {
    const firstString = value.find(item => typeof item === 'string' && item.trim())
    return typeof firstString === 'string' ? firstString : undefined
  }

  return undefined
}

function resolveComingSoonProps(query: Record<string, unknown>) {
  const requestedPath = normalizeStudentPath(readSingleQueryValue(query.from) ?? '')

  return {
    title: localizeText(readSingleQueryValue(query.title) ?? COMING_SOON_TOAST),
    subtitle: localizeText(STUDENT_COMING_SOON_SUBTITLE),
    requestedPath: requestedPath === '/' ? '' : requestedPath
  }
}

const routes: RouteRecordRaw[] = [
  {
    path: '/login',
    name: 'Login',
    component: () => import('@/views/login/index.vue'),
    meta: { title: "登录", public: true } // i18n-skip-line: route meta title
  },
  {
    path: '/forgot-password',
    name: 'ForgotPassword',
    component: () => import('@/views/forgot-password/index.vue'),
    meta: { title: "找回密码", public: true } // i18n-skip-line: route meta title
  },
  {
    path: '/account-locked',
    name: 'AccountLocked',
    component: () => import('@/views/account-locked/index.vue'),
    // public 让未登录也能直达；rolloutBypass 跳过 isStudentPathAvailable 灰度过滤 // i18n-skip-line: route meta title
    meta: { title: "账号已锁定", public: true, rolloutBypass: true } // i18n-skip-line: route meta title
  },
  {
    path: '/',
    component: MainLayout,
    redirect: PHASE1_DEFAULT_PATH,
    children: [
      {
        path: 'dashboard',
        name: 'Dashboard',
        component: () => import('@/views/dashboard/index.vue'),
        meta: { title: "首页" } // i18n-skip-line: route meta title
      },
      {
        path: 'home',
        name: 'Home',
        component: () => import('@/views/dashboard/index.vue'),
        meta: { title: "首页" } // i18n-skip-line: route meta title
      },
      {
        path: 'courses',
        name: 'Courses',
        component: () => import('@/views/courses/index.vue'),
        meta: { title: "课程记录" } // i18n-skip-line: route meta title
      },
      {
        path: 'myclass',
        name: 'MyClass',
        component: () => import('@/views/courses/index.vue'),
        meta: { title: "课程记录" } // i18n-skip-line: route meta title
      },
      {
        path: 'communication',
        name: 'Communication',
        component: () => import('@/views/communication/index.vue'),
        meta: { title: "人际关系沟通记录" } // i18n-skip-line: route meta title
      },
      {
        path: 'feedback',
        name: 'Feedback',
        component: () => import('@/views/feedback/index.vue'),
        meta: { title: "课程反馈" } // i18n-skip-line: route meta title
      },
      {
        path: 'report',
        name: 'Report',
        component: () => import('@/views/report/index.vue'),
        meta: { title: "上课记录" } // i18n-skip-line: route meta title
      },
      {
        path: 'ai-interview',
        name: 'AiInterview',
        component: () => import('@/views/ai-interview/index.vue'),
        meta: { title: "AI面试分析" } // i18n-skip-line: route meta title
      },
      {
        path: 'resume',
        name: 'Resume',
        component: () => import('@/views/resume/index.vue'),
        meta: { title: "简历管理" } // i18n-skip-line: route meta title
      },
      {
        path: 'ai-resume',
        name: 'AiResume',
        component: () => import('@/views/ai-resume/index.vue'),
        meta: { title: "AI简历分析" } // i18n-skip-line: route meta title
      },
      {
        path: 'career',
        name: 'Career',
        component: () => import('@/views/career/index.vue'),
        meta: { title: "求职中心" } // i18n-skip-line: route meta title
      },
      {
        path: 'positions',
        name: 'Positions',
        component: () => import('@/views/positions/index.vue'),
        meta: { title: "岗位信息" } // i18n-skip-line: route meta title
      },
      {
        path: 'applications',
        name: 'Applications',
        component: () => import('@/views/applications/index.vue'),
        meta: { title: "我的求职" } // i18n-skip-line: route meta title
      },
      {
        path: 'job-tracking',
        name: 'JobTracking',
        component: () => import('@/views/applications/index.vue'),
        meta: { title: "我的求职" } // i18n-skip-line: route meta title
      },
      {
        path: 'mock-practice',
        name: 'MockPractice',
        component: () => import('@/views/mock-practice/index.vue'),
        meta: { title: "面试测试" } // i18n-skip-line: route meta title
      },
      {
        path: 'request',
        name: 'Request',
        component: () => import('@/views/mock-practice/index.vue'),
        meta: { title: "课程申请" } // i18n-skip-line: route meta title
      },
      {
        path: 'files',
        name: 'Files',
        component: () => import('@/views/files/index.vue'),
        meta: { title: "文件" } // i18n-skip-line: route meta title
      },
      {
        path: 'online-test-bank',
        name: 'OnlineTestBank',
        component: () => import('@/views/online-test-bank/index.vue'),
        meta: { title: "在线测试题库" } // i18n-skip-line: route meta title
      },
      {
        path: 'interview-bank',
        name: 'InterviewBank',
        component: () => import('@/views/interview-bank/index.vue'),
        meta: { title: "真人面试题库" } // i18n-skip-line: route meta title
      },
      {
        path: 'questions',
        name: 'Questions',
        component: () => import('@/views/questions/index.vue'),
        meta: { title: "面试真题" } // i18n-skip-line: route meta title
      },
      {
        path: 'profile',
        name: 'Profile',
        component: () => import('@/views/profile/index.vue'),
        meta: { title: "个人中心" } // i18n-skip-line: route meta title
      },
      {
        path: 'coming-soon',
        name: STUDENT_COMING_SOON_ROUTE_NAME,
        component: () => import('@/views/placeholder/index.vue'),
        props: route => resolveComingSoonProps(route.query),
        meta: { title: COMING_SOON_TOAST, rolloutBypass: true }
      },
      {
        path: 'notice',
        name: 'Notice',
        component: () => import('@/views/notice/index.vue'),
        meta: { title: "消息中心" } // i18n-skip-line: route meta title
      },
      {
        path: 'faq',
        name: 'Faq',
        component: () => import('@/views/faq/index.vue'),
        meta: { title: "常见问题" } // i18n-skip-line: route meta title
      },
      {
        path: 'complaint',
        name: 'Complaint',
        component: () => import('@/views/complaint/index.vue'),
        meta: { title: "投诉与建议" } // i18n-skip-line: route meta title
      },
      {
        path: 'netlog',
        name: 'Netlog',
        component: () => import('@/views/netlog/index.vue'),
        meta: { title: "沟通记录" } // i18n-skip-line: route meta title
      },
      {
        path: 'restricted',
        name: 'Restricted',
        component: () => import('@/views/restricted/index.vue'),
        meta: { title: "受限模式", rolloutBypass: true } // i18n-skip-line: route meta title
      },
      {
        path: 'schedule',
        name: 'Schedule',
        component: () => import('@/views/schedule/index.vue'),
        meta: { title: "课程排期" } // i18n-skip-line: route meta title
      },
      {
        path: 'resources',
        name: 'Resources',
        component: () => import('@/views/resources/index.vue'),
        meta: { title: "学习资源" } // i18n-skip-line: route meta title
      }
    ]
  }
]

const router = createRouter({
  history: createWebHistory(),
  routes
})

// 路由守卫 // i18n-skip-line: route meta title
router.beforeEach((to, _from, next) => {
  const token = getToken()

  if (to.meta.public) {
    next()
  } else if (!token) {
    next({ name: 'Login', query: { redirect: to.fullPath } })
  } else if (isAccountLocked(to)) {
    next({ name: 'AccountLocked', query: { reason: resolveLockReason() } })
  } else if (to.path === '/' || to.meta.rolloutBypass) {
    next()
  } else if (!isStudentPathAvailable(to.path)) {
    next({
      name: STUDENT_COMING_SOON_ROUTE_NAME,
      query: {
        from: normalizeStudentPath(to.path),
        title: typeof to.meta.title === 'string' ? to.meta.title : COMING_SOON_TOAST
      }
    })
  } else {
    next()
  }
})

/**
 * 判断当前学员是否处于锁定态：account_status=2（已结束）或 黑名单。 // i18n-skip-line: route meta title
 *
 * 守卫触发时 user 信息通常已由 login-workflow 写入 localStorage。 // i18n-skip-line: route meta title
 * 极端 case：若 getUser 返回 null（未走完登录、刷新前老 token），不拦截， // i18n-skip-line: route meta title
 * 让后续接口由后端 ServiceException 兜底。 // i18n-skip-line: route meta title
 */
function isAccountLocked(to: { name?: string | symbol | null }): boolean {
  if (to.name === 'AccountLocked') {
    return false
  }
  const user = getUser<{ accountStatus?: string; blacklisted?: boolean } | null>()
  if (!user) {
    return false
  }
  return user.accountStatus === '2' || user.blacklisted === true
}

function resolveLockReason(): 'contract_ended' | 'blacklisted' {
  const user = getUser<{ accountStatus?: string; blacklisted?: boolean } | null>()
  if (user?.accountStatus === '2') {
    return 'contract_ended'
  }
  return 'blacklisted'
}

export default router
