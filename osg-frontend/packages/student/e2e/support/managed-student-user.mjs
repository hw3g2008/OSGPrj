...
const CLASS_DIR = path.join(os.tmpdir(), 'student-managed-e2e-java')
const SOURCE_FILE = path.resolve(CURRENT_DIR, 'java/CreateManagedStudentPortalUser.java')
...
function compileManagedStudentUser() {
...
export function reseedManagedStudentPortalUser(password = 'student12766') {
  hydrateDbEnvFromRuntimeFile()
  const classDir = compileManagedStudentUser()

  for (let attempt = 1; attempt <= 3; attempt += 1) {
    try {
      execFileSync(
        'java',
        ['-cp', `${classDir}${path.delimiter}${resolveSeedClasspath()}`, 'CreateManagedStudentPortalUser', password],
        {
          stdio: 'pipe',
          env: process.env,
        },
