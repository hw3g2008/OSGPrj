...
describe('全部课程页面', () => {
  it('renders the all-classes shell with five tabs, six modal variants, and pagination copy', () => {
    ...
    expect(pageSource).toContain('条记录')
    ...
    expect(modalSource).toContain('feedback-panel--entry')
    expect(modalSource).toContain('feedback-panel--mock')
    expect(modalSource).toContain('feedback-panel--midterm')
    expect(modalSource).toContain('feedback-panel--networking')
    expect(modalSource).toContain('feedback-panel--written')
    expect(modalSource).toContain('feedback-panel--rejected')
  })
