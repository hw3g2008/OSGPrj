...
describe('岗位信息页面原型收口', () => {
  ...
  it('uses the compact prototype filter bar with publish preset and inline search', () => {
    ...
    expect(filtersBlock).toContain('展示起始')
  ...
  it('keeps recruitment cycles on formal business values instead of bare years', () => {
    ...
    expect(serviceSource).toContain('private static final String DICT_RECRUITMENT_CYCLE = "osg_recruit_cycle";')
    expect(serviceSource).toContain('meta.put("recruitmentCycles", buildStaticOptions(DICT_RECRUITMENT_CYCLE));')
    expect(serviceSource).not.toContain('new DictSeed(DICT_RECRUITMENT_CYCLE,')
    expect(serviceSource).not.toContain('purgeObsoleteDictValues(DICT_RECRUITMENT_CYCLE')
    expect(serviceSource).not.toContain('normalizeLegacyRecruitmentCycles')

    expect(e2eSource).toContain("recruitmentCycle: overrides.recruitmentCycle ?? 'Class of 2026'")
    expect(e2eSource).toContain("recruitmentCycle: 'Class of 2026'")
    expect(e2eSource).not.toContain("recruitmentCycle: overrides.recruitmentCycle ?? '2026 Summer'")
  })
})
