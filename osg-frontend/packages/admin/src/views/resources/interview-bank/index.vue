<template>
  <div class="osg-page">
    <PageHeader :title-zh="$t('real_person_interview_question_bank')" title-en="Interview Bank" :description="$t('manage_interview_question_collections_ca')">
      <template #actions>
    ...
    <a-alert
      ...
      type="info"
      show-icon
      :message="$t('question_bank_assignment_pending_count', { count: pendingCount })"
      :description="`${$t('please_prioritize_interview_question_ban')}。`"
    >
      <template #action>
        <a-button size="small" @click="activeTab = 'applications'">{{ $t('view_requests') }}</a-button>
      </template>
    </a-alert>
    ...
    <a-card :bordered="false">
      <a-tabs v-model:activeKey="activeTab" style="margin-bottom: 16px">
        <a-tab-pane key="banks">
          <template #tab>{{ $t('question_bank_list') }}</template>
        </a-tab-pane>
      ...
      <a-form layout="inline" style="margin-bottom: 16px; gap: 12px; flex-wrap: wrap">
        <a-form-item>
          <a-input v-model:value="filters.keyword" :placeholder="activeTab === 'banks' ? $t('search_question_bank_industry') : $t('search_student_position')" allow-clear style="width: 180px" @press-enter="loadRows" />
        </a-form-item>
        <template v-if="activeTab === 'banks'">
          <a-form-item>
            <a-select v-model:value="filters.interviewStage" :placeholder="$t('all_stages')" allow-clear style="width: 140px">
              <a-select-option v-for="option in interviewStageOptions" :key="option" :value="option">{{ option }}</a-select-option>
            </a-select>
          </a-form-item>
          <a-form-item>
            <a-select v-model:value="filters.interviewType" :placeholder="$t('all_types')" allow-clear style="width: 120px">
              <a-select-option v-for="option in interviewTypeOptions" :key="option" :value="option">{{ option }}</a-select-option>
            </a-select>
          </a-form-item>
          <a-form-item>
            <a-select v-model:value="filters.industryName" :placeholder="$t('all_industries')" allow-clear style="width: 140px">
      ...
      <a-table v-if="activeTab === 'banks'" :columns="bankColumns" :data-source="rows" :row-key="(r: InterviewBankRow) => r.bankId" :pagination="false" :locale="{ emptyText: $t('no_question_bank_records') }" :scroll="{ x: 1000 }">
      ...
      <a-table v-else :columns="applicationColumns" :data-source="rows" :row-key="(r: InterviewBankRow) => r.applicationCode" :pagination="false" :locale="{ emptyText: $t('no_student_requests') }" :scroll="{ x: 900 }">
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.dataIndex === 'applicationSource'">
            <a-tag color="blue">{{ record.applicationSource || $t('forwarded_by_class_advisor') }}</a-tag>
          </template>
          <template v-else-if="column.dataIndex === 'appAction'">
            <a-button type="primary" size="small">{{ $t('assign_question_bank') }}</a-button>
          </template>
        </template>
      </a-table>
    </a-card>
...
const bankColumns = [
  { title: t('question_bank_name'), dataIndex: 'interviewBankName', key: 'interviewBankName', width: 200 },
  { title: t('stage'), dataIndex: 'interviewStage', key: 'interviewStage', width: 120 },
  { title: t('type'), dataIndex: 'interviewType', key: 'interviewType', width: 100 },
  { title: t('industry'), dataIndex: 'industryName', key: 'industryName', width: 130 },
  ...
  { title: t('updated_at'), dataIndex: 'updatedAt', key: 'updatedAt', width: 130 },
  { title: t('operation'), dataIndex: 'action', key: 'action', width: 80 },
]

const applicationColumns = [
  { title: t('application_id'), dataIndex: 'applicationCode', key: 'applicationCode', width: 120 },
  { title: t('student'), dataIndex: 'studentName', key: 'studentName', width: 120 },
  { title: t('applied_position'), dataIndex: 'appliedPosition', key: 'appliedPosition', width: 150 },
  { title: t('stage'), dataIndex: 'interviewStage', key: 'interviewStage', width: 120 },
  { title: t('application_time'), dataIndex: 'applicationTime', key: 'applicationTime', width: 130 },
  { title: t('source'), dataIndex: 'applicationSource', key: 'applicationSource', width: 110 },
  { title: t('operation'), dataIndex: 'appAction', key: 'appAction', width: 100 },
]
...
const loadRows = async () => {
  try {
    ...
    message.error(activeTab.value === 'banks' ? t('failed_to_load_question_bank_list') : t('failed_to_load_student_requests'))
  }
}
...
const handleSubmit = async (payload: SaveInterviewBankPayload) => {
  ...
  try {
    if (formMode.value === 'edit' && editingRow.value?.bankId) {
      ...
      message.success(t('question_bank_updated_successfully'))
    } else {
      await createInterviewBank(payload)
      message.success(t('question_bank_created_successfully'))
    }
