<template>
  <div id="page-schedule" class="page-schedule" data-page="assistant-schedule">
    ...
    <section class="card">
      ...
      <div class="card-body">
        <div class="form-group">
          ...
          <div class="hours-row">
            <input
              id="assistant-next-weekly-hours"
              v-model="weeklyHours"
              type="number"
              min="0"
              max="40"
...
async function loadScheduleViews(preserveDraft = false) {
  try {
    const [currentView, nextView, nextStatus] = await Promise.all([
      assistantScheduleApi.getSchedule('current'),
      assistantScheduleApi.getSchedule('next'),
      assistantScheduleApi.getStatus(),
    ])
...
async function saveNextSchedule() {
  try {
    await assistantScheduleApi.saveNext({
