/**
 * asst login 页 forgot-password 集成契约测试
 *
 * M6 P5 阶段3：asst 从独立 page 纠偏为 login 内嵌 modal（严格按原型 SSOT）。
 * 业务流核心断言（4 step 切换 + endpoints 调用）由 shared/components/ForgotPasswordModal.spec.ts (12 case) 覆盖。
 * 本 spec 仅验证 asst login.vue 正确集成 shared 组件 + endpoints 注入正确。
 */
...
describe('asst login.vue × ForgotPasswordModal 集成契约', () => {
  ...
  it('2. login.vue 注入了 endpoints 且调用 asst 端 API', () => {
    expect(loginViewSource).toContain('sendResetCode')
    expect(loginViewSource).toContain('verifyResetCode')
    expect(loginViewSource).toContain('resetPassword')
  ...
  it('3. login.vue 不再链接到独立 forgot-password page (router-link to=/forgot-password)', () => {
