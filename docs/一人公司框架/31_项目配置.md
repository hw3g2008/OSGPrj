...
本文件是说明文档，不是第二规则源。
如果本文档与 `config.yaml` 冲突，以 `config.yaml` 为准。

进入任何新窗口、新机器或新执行阶段前，先跑：
...
- `ui_delivery_policy`
  - 定义 UI 严格交付的机器真值
  - 默认零容忍
    - screenshot compare 先严格执行
    - 不允许用宽松 `diff_threshold` 代替精确分类
  - 正式修复链路固定为：
    - `single_case_verify`
    - `module_verify`
    - `ui_visual_gate`
...
**移植框架到新项目的核心步骤**：
1. 复制 `.claude/` 和 `.claude/commands/`（通用部分）
...
paths:
  ...
  tasks:
    root: "{tasks_root}"         # 如 tasks/、docs/tasks/
    state: "{tasks_root}/STATE.yaml"
    stories: "{tasks_root}/stories/"
    tickets: "{tasks_root}/tickets/"
