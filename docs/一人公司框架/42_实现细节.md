...
## Step 1: 读取状态
1. 执行: `读取 osg-spec-docs/tasks/STATE.yaml`
...
2. 获取: `type` 字段（backend | frontend | database）

## Step 3: 确定 Agent
1. 根据 type 映射:
   - backend → 读取 `.claude/project/agents/backend-java.md`
   - frontend → 读取 `.claude/project/agents/frontend-vue.md`
   - database → 读取 `.claude/project/agents/dba-mysql.md`
2. 读取 Agent 文件中的:
