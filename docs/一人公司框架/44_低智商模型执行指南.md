...
def 找下一个Ticket() -> str | None:
    ...
    if state["current"]["ticket"]:
        ...
        ticket = 读取yaml(f"tasks/tickets/{ticket_id}.yaml")
...
**精确读取路径**：`tasks/tickets/{ticket_id}.yaml`
...
steps:
  - step: 1
    ...
    duration_ms: 100
    details: "读取 tasks/tickets/T-003.yaml"
