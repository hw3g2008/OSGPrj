---
name: 一人公司 AI 交付框架
overview: 基于 obra/superpowers、Ralph Loop、Catalyst、TÂCHES、LangGraph 等业界最佳实践，独立实现一套 Skills + Agents 结合的工程化交付框架。支持记忆管理、上下文压缩、自主循环、检查点恢复，分离通用层与项目层，支持多端并行开发。
todos:
  - id: phase1-core-structure
    content: 创建 core/ 通用框架目录结构
    status: in_progress
  - id: phase2-memory-skills
    content: "编写记忆管理 Skills: memory-bank, context-compression, checkpoint-manager"
    status: pending
  - id: phase3-workflow-skills
    content: "编写工作流 Skills: brainstorming, story-splitter, ticket-splitter, deliver-ticket"
    status: pending
  - id: phase4-quality-skills
    content: "编写质量 Skills: verification, tdd, debugging, code-review"
    status: pending
  - id: phase5-automation-skills
    content: "编写自动化 Skills: ralph-loop, progress-tracker"
    status: pending
  - id: phase6-agent-templates
    content: "编写 Agent 模板: architect, planner, developer, reviewer, qa, coordinator"
    status: pending
  - id: phase7-project-config
    content: 创建 project/ 项目配置结构 + 若依项目实例
    status: pending
  - id: phase8-commands
    content: 编写 commands/*.md 快捷命令
    status: pending
  - id: phase9-e2e-test
    content: "端到端测试: 需求 → /brainstorm → /split → /next → /verify"
    status: pending
---
...
| **Skill** | 岗位职责手册 | 定义“怎么做”：规则、步骤、约束 | `skills/` |

| **Agent** | 员工 | 定义“谁来做”：角色、工具、加载哪些 Skills | `agents/` |
...
```mermaid
flowchart TD
    Main[主分支 main] --> WT1[Worktree: feature-user<br/>Backend Agent<br/>处理用户模块]
    Main --> WT2[Worktree: feature-order<br/>Frontend Agent<br/>处理订单页面]
    Main --> WT3[Worktree: hotfix-auth<br/>Debug Agent<br/>修复认证问题]
