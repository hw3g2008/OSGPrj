...
1. 一批历史 gap 已经不再匹配当前代码，尤其是 Student 课程记录回看链、Student 求职主链、Student `practice-request` 模拟应聘主链。
2. 真正新增且最危险的问题集中在 Admin 分配链，且都不是“接口不存在”，而是“接口成功但写入错误导师标识，导致导师端看不到分配结果”。
3. Assistant 端仍存在功能侧缺口，尤其是课时记录只读不提交流。
4. 需求资产本身还有一批冲突和缺口，如果不先统一，后续修复容易再次出现“实现正确但验收口径不一致”。
...
- `结论`：
  恢复链实现总体可用，但跨端命名空间不统一，且 Admin forgot-password surface 仍存在需求资产冲突。
...
- `结论`：
  字典接入不是“全局未做”，而是“业务主链大多已动态化，profile 相关页面仍明显落后”，并且需求资产层面还保留 `基础数据统一源 vs 页面静态选项` 的冲突。

## 9. 下一步

建议直接进入修复设计和优先级排程，不需要再继续同层级的大范围静态审计。

推荐修复顺序：

1. `P1` 新增问题优先：
   - Admin 求职分配导师链使用非真实导师标识
   - Admin 模拟应聘分配链使用非真实导师标识
   - Assistant 课时提交侧缺失
2. `P0 / P1` 已知问题其次：
   - Student 审核字段链断裂
   - Staff 审批侧缺失
...
3. `需求冲突` 并行清理：
   ...
   - Lead-Mentor 可编辑字段范围
   - 教学链提交来源范围
   - Admin forgot-password surface
   - 基础数据统一源口径
4. `存疑` 项单独归档：
   - 缺 PRD 资产的链路先补需求契约，再决定是否算实现问题
