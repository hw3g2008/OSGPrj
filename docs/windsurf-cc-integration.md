...
| /split ticket | ✅ 6项质量 | ✅ AC↔Ticket 100% | ✅ 5轮 | ✅ | ❌ 缺失 | auto |
...
2. **`/verify` 缺多轮迭代和失败退出** — 验收不通过时处理逻辑不够明确
3. **所有环节缺少 Phase 3（全局终审）** — 修复后只重跑检查项，没有从全局角度重新审视
...
> 详细修改方案（细化到每一行代码）见 [`docs/gate-control-fix-plan.md`](gate-control-fix-plan.md)

| # | 待实施项 | 对应修复 | 优先级 |
|---|---------|---------|--------|
| 1 | 补充 `/brainstorm` 的 PRD 覆盖率校验 | fix-plan #1 | 🔴 高 |
| 2 | 补充 `/verify` 的多轮迭代 + 自动修复循环 | fix-plan #7 | 🔴 高 |
| 3 | 补充 `/verify` 的 Story AC 覆盖率校验 | fix-plan #6 | 🔴 高 |
| 4 | 补充 `/verify` 的明确失败退出逻辑 | fix-plan #8 | 🟡 中 |
| 5 | 为 `/brainstorm` 添加 Phase 3 全局终审 | fix-plan #2 | 🟡 中 |
| 6 | 为 `/split story` 添加 Phase 3 全局终审 | fix-plan #3 | 🟡 中 |
