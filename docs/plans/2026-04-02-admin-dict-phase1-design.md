...
Non-goal: 本期不改学生/导师/班主任/助教端下拉接法，不在本期重做动态权限模块，不在本期一次性完成所有页面字典化，不在本期改全局 RPIV 状态机。
...
5. 在 `admin-dict` 内局部试行 `Story -> Slice -> Ticket`，但不提升为全局 workflow 层。
...
本次确认在 `admin-dict` requirement 内试行一层 `story_slices`，但它的定位必须严格受限：

1. `story_slices` 只在 `admin-dict` requirement 内启用
2. 不修改全局 `RPIV` 主链定义
3. 不进入全局 `state-machine.yaml`
4. 不新增全局命令如 `/split slice`
5. 不新增全局审批节点

也就是说：

- 全局主链仍然是 `Requirement -> Story -> Ticket`
- `admin-dict` 只是让 `ticket-splitter` 在本地先按 slice 分组，再在 slice 下生成 ticket

这样做的目的不是引入新的流程负担，而是验证“复杂 Story 先做行为切片再出 Ticket”是否能明显降低 Ticket 偏移。
...
`story_slices` 采用 **Story 内嵌元数据** 形式，而不是新建一层独立文件或状态节点。
...
story_slices:
  - slice_id: SD-S-001-01
    title: Registry 真相与种子
    goal: 固化 dict type 分组、父子关系与首批字典种子
    covers_ac_refs:
      - AC-S-001-01
  ...
  - slice_id: SD-S-001-02
    ...
    goal: 让 basedata 兼容接口委托标准 dict 真源
    covers_ac_refs:
      - AC-S-001-02
    allowed_ticket_types:
      - backend
...
1. 每个 Ticket 必须挂到 1 个 `slice_id`
2. `slice` 只是 Ticket 的分组与约束来源，不单独执行
3. `slice` 不允许写自己的 `status` 到全局 `STATE.yaml`
4. `slice` 的完成度由其下 Ticket 聚合得出，不需要单独审批

适用前提：

1. Story 同时包含多个相对独立的行为链
2. Ticket 直接从 Story 生成时容易跨链混写
3. 先切片能把 docs/backend/frontend/compat 这些边界收清楚
...
- Story YAML
- `story_slices`（仅 `admin-dict` requirement 内启用的 Story 内嵌切片元数据）
- Ticket YAML
...
5. `story_slices` 只能服务 `admin-dict` Story 内部分解，不得反向污染全局 workflow
...
`admin-dict` 的 `story_slices` 试行只增加 **局部分解约束**，不增加全局门禁节点。

局部要求：

1. Story 若声明了 `story_slices`，则每个 Ticket 必须带 `slice_id`
2. 每个 slice 至少要有 1 个 Ticket
3. `covers_ac_refs` 不能出现“某个 slice 无 AC 承接”

不做：

1. 不为 slice 新增 `/approve slice`
2. 不为 slice 新增全局状态
3. 不为 slice 新增独立 verify 节点
...
8. `admin-dict` 的 `story_slices` 能在不修改全局状态机的前提下，清晰约束 Ticket 分组与承接关系。
