...
> 状态：§三 数据/字典层 ✅ 已完成（2026-04-20）| §四 测试同步 ✅ 已完成 | §二 UI 层 🚫 受阻于 T1/T2
...
> **完成于** 2026-04-20 | commit `3ccdcd6b`
...
### 3.3 ✅ 导师主攻方向（临时处理：默认值清空）

> **完成于** 2026-04-20 | commit `3ccdcd6b` | 默认值 `ref('ib')` → `ref('')`，业务决策（选 A/B）待后续
...
## 四、✅ 测试同步（2026-04-20 完成）

> **全量结果**：114/114 passed | commits `3ccdcd6b` + `4831f801`

| 真实测试文件 | 改动要点 | 状态 |
|---|---|---|
| `positions-shell.spec.ts` | mock `meta.industries` 改新 7 项 `PositionMetaOption[]`；断言 CSS class `industry-{tone}` | ✅ |
| `positions-real-flow.spec.ts` | 加 `@osg/shared` mock 防 localStorage 报错 | ✅ |
| `story-s041-regression.spec.ts` | 同上 | ✅ |
| `story-s040-regression.spec.ts` | sidebar 断言改为 V1 可见 groups；toast 触发改用"个人设置" | ✅ |
| `upcoming-navigation-toast.spec.ts` | 同上 | ✅ |
