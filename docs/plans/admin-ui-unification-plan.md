...
### Phase 1: 改 B-ANT 页面（5页，已部分使用 Ant，改动最小）
...
### Phase 2: 改 A-NEW 标杆页面（6页，原生→Ant）

| 步骤 | 页面 | 行数 | 改动要点 |
|------|------|------|----------|
| 2-1 | permission/roles | 536 | 原生 table→a-table，原生 button→a-button，header→PageHeader |
| 2-2 | permission/users | 1192 | 同上，表单较多 |
| 2-3 | users/staff | 1467 | 同上，最大标杆页，筛选栏+表格+弹窗全改 |
| 2-4 | users/students | 2041 | 同上，最大页面 |
| 2-5 | users/contracts | 1023 | 同上 |
| 2-6 | career/job-tracking | 618 | 同上 |

### Phase 3: 改 D-MIX 页面（10页）

| 步骤 | 页面 | 行数 | 改动要点 |
|------|------|------|----------|
| 3-1 | dashboard | 167 | 首页特殊布局，加 PageHeader + 用 a-card 包装 |
| 3-2 | teaching/communication | 289 | header→PageHeader，原生→Ant |
| 3-3 | permission/base-data | 601 | 混合(a-input+原生table)→统一 Ant |
| 3-4 | teaching/class-records | 723 | 原生→Ant |
| 3-5 | teaching/reports | 714 | 原生→Ant |
| 3-6 | teaching/feedback | 633 | 大量原生 select/input→Ant |
| 3-7 | career/student-positions | 747 | 原生→Ant |
| 3-8 | career/mock-practice | 894 | 原生→Ant |
| 3-9 | career/job-overview | 1263 | 大页面，原生→Ant |
| 3-10 | users/mentor-schedule | 750 | 原生→Ant |

### Phase 4: 改 C-RAW 页面（9页）

| 步骤 | 页面 | 行数 | 改动要点 |
|------|------|------|----------|
| 4-1 | resources/files | 275 | eyebrow/ghost-button→PageHeader+Ant |
| 4-2 | resources/qbank | 280 | 同上 |
| 4-3 | finance/expense | 377 | 同上 |
| 4-4 | resources/interview-bank | 493 | 同上 |
| 4-5 | resources/online-test-bank | 505 | 同上 |
| 4-6 | resources/questions | 568 | 同上 |
| 4-7 | finance/settlement | 522 | 同上，有统计卡片和 Tab 切换 |
| 4-8 | teaching/all-classes | 777 | ghost-button→Ant |
| 4-9 | career/positions | 1624 | 最大 C-RAW 页面 |

### Phase 5: 清理

| 步骤 | 内容 |
|------|------|
| 5-1 | 删除 global.scss 中不再使用的自定义样式（如果有） |
| 5-2 | 检查是否还有残留的 `permission-button`、`ghost-button` 等旧 class |
| 5-3 | 检查 mdi 图标引用，确认是否可以移除 `@mdi/font` 依赖 |
| 5-4 | 浏览器全页面验证 |
