...
**Architecture:** 新建 `admin-dict` source-stage 资产和 `DICT-REGISTRY.yaml` 作为机器真相；在 `admin-dict` Story 内局部试行 `story_slices` 分组，但不改全局状态机；后端新增 registry 读取能力并把 `/system/basedata/*` 改成兼容 facade；前端保留 `/permission/base-data` 入口，但页面和弹窗改为 registry + 标准 dict 驱动。
...
Status: Draft  
Owner: admin-dict  
Scope: admin 端基础数据管理页、标准字典真源、兼容 facade、独立 RPIV requirement、局部 story slice 试行  
Non-goal: 其他端和其他页面下拉接字典、动态权限菜单树重构、全量移除 `/system/basedata/*`、改全局 RPIV 状态机

**Design Doc:** `docs/plans/2026-04-02-admin-dict-phase1-design.md`

**Execution Order:** source docs -> slice trial rules -> registry seed -> backend registry/facade -> frontend page refactor -> compatibility tests -> RPIV asset bootstrap
...
7. 兼容 smoke + 新页面测试通过。
8. `admin-dict` 在不改全局状态机的前提下完成 `Story -> Slice -> Ticket` 局部试行。
...
- Modify: `osg-spec-docs/tasks/stories/*.yaml` (仅 `admin-dict` Story，新增 `story_slices`)
- Modify: `osg-spec-docs/tasks/tickets/*.yaml` (仅 `admin-dict` Ticket，新增 `slice_id`)
...
### Task 1.5: Add Local Slice Trial Rules for `admin-dict`

**Files:**
- Modify: `osg-spec-docs/docs/02-requirements/srs/admin-dict.md`
- Modify: `osg-spec-docs/tasks/stories/*.yaml` (generated later)
- Modify: `osg-spec-docs/tasks/tickets/*.yaml` (generated later)

- [ ] **Step 1: Add slice rule section to SRS**

```markdown
### FR-006 Story Slice Trial

- REQ-006.1: `admin-dict` Story 可声明 `story_slices`
- REQ-006.2: 每个 Ticket 必须归属 1 个 `slice_id`
- REQ-006.3: `story_slices` 仅用于 `admin-dict` 本地试行，不进入全局状态机
```

- [ ] **Step 2: Define expected slice split**

```yaml
story_slices:
  - slice_id: SD-S-001-01
    title: Registry 与字典种子
  - slice_id: SD-S-001-02
    title: Backend Facade
  - slice_id: SD-S-001-03
    title: Frontend 页面
  - slice_id: SD-S-001-04
    title: Compatibility 与验证
```

- [ ] **Step 3: Verify no global workflow files are touched**

Run:

```bash
git diff --name-only | rg 'state-machine|workflow-engine|commands/.*/split'
```
...
```bash
git add osg-spec-docs/docs/02-requirements/srs/admin-dict.md
git commit -m "docs: define admin-dict local slice trial"
```
...
- [ ] **Step 3: Verify operation obligations and slice metadata exist**
...
for path in glob.glob('osg-spec-docs/tasks/stories/S-*.yaml'):
    ...
    if data.get('requirements') and 'REQ-001' in ''.join(data.get('requirements', [])):
        ...
        print(path, data.get('story_slices'))
PY
```

- [ ] **Step 3.5: Ensure every generated ticket has `slice_id`**
...
missing = []
for path in glob.glob('osg-spec-docs/tasks/tickets/T-*.yaml'):
    data = yaml.safe_load(open(path, 'r', encoding='utf-8'))
    if data.get('story_id') and data.get('story_id').startswith('S-'):
        if data.get('requirements') or data.get('slice_id') is not None:
            pass
    if path and 'admin-dict' in path:
...
Expected: PASS with no missing `slice_id` for `admin-dict` tickets.
