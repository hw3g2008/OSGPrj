...
1. 优先复用已启动后端（`/actuator/health` 可达）
   - 开发态唯一正确入口：`bash bin/run-backend-dev.sh deploy/.env.dev`
   - 开发态 runtime 真源：`deploy/runtime-contract.dev.yaml`
2. 若不可达，按 `--backend-policy` 执行：
   - `docker_only`：必须走 Docker 启动（禁止回退本地托管 backend）
   - `auto`：优先 Docker，Docker 不可用时回退本地托管 `bash bin/run-backend-dev.sh deploy/.env.dev`
3. Docker 启动命令解析优先级：
   - 环境变量 `DOCKER_RUN_CMD`
...
4. 代理/连通性错误兜底（命中即失败）：
   - `http proxy error`
   - `ECONNREFUSED`
5. 忽略第三方告警（如 Sass/Vite `DEPRECATION WARNING`）
6. `final-gate` 固定包含安全契约守卫：
   - `security_contract_guard.py --stage final-gate`
   - `delivery_truth_guard.py --stage final-gate`
   - `delivery_content_guard.py --stage final-gate`
   - 产物：`osg-spec-docs/tasks/audit/security-contract-{module}-{date}.md`
...
- `osg-spec-docs/tasks/audit/e2e-api-gate-{module}-{date}.log`
- `osg-spec-docs/tasks/audit/security-contract-{module}-{date}.md`
