...
8. **逐项校验**
   ...
   - 对每个修改点提问：
     ...
     - 修改是否与其他文件矛盾？
   - **强制回验**（每批修改后必做）：
     - 数值回验：重新计数文档中所有“N 个 X”，与实际表格行数对比
     - 引用回读：对本批新增/修改的每个外部引用（状态名、命令、行号），回读源文件验证
     - 反向推导：从目标产物反向检查执行清单是否覆盖所有输入源
...
- 推荐的维度轮换：
  ...
  - 第10轮：J 数值一致性（计数与实际内容匹配）
  - 第11轮：K 源文件回读（引用值与源文件一致）
...
| **J 数值一致性** | 文档中的计数与实际内容是否匹配 | “10 个写入点”但表格只有 9 行？“5 个决策”但只列了 4 个？ |
| **K 源文件回读** | 引用的外部值是否与源文件一致 | 写了“命令是 /xxx”但源文件实际是 /yyy？状态名与源码不符？ |
...
| G10 | 数值回验 | 文档中出现的所有计数（“N 个 X”）是否与实际列表/表格行数精确匹配？修改表格后是否重新计数？ |
| G11 | 引用回读 | 方案中引用的每个外部值（状态名、命令写法、文件行号、规则描述）是否回读源文件验证过？禁止凭记忆或复制粘贴。 |
| G12 | 反向推导 | 从目标产物（如“所有写状态的文件都要 X”）反向推导所有输入源，确认执行清单无遗漏？ |
